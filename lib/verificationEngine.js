/**
 * MedVerify — Phase 3: Verification Engine + Confidence Scoring
 *
...
export async function verifyMedication({
  ...
  if (!medication && medication_name) {
    // Fallback: fuzzy match on inn_name or brand_names
    ...
    if (medByName) {
      ...
      // Partial score for name-only match (no GTIN confirmation)
